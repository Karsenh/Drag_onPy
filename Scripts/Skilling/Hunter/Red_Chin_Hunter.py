...
def color_exists(region, color, img_name):
    ...
    return does_color_exist_in_sub_image(region, color, img_name, color_tolerance=38, count_min=5)
...
def handle_green_found(curr_check_trap_num, curr_at_trap_num, curr_check_trap_claim_coords, curr_check_trap_tile_coords, spot_emojis):
    ...
    if did_click_check_trap:
        ...
        if saw_exp_drop:
            ...
            print(f'Failed to find exp drop - but we will assume to have reset trap by checking if the trap is set.')
            return trap_is_set(curr_check_trap_num)
...
def trap_is_set(curr_check_trap_num):
    ...
    while not found_yellow and search_attempts <= 100:
        ...
        update_curr_trap_state(curr_check_trap_num)
        found_yellow = does_color_exist_in_sub_image(yellow_check_region, yellow_color, 'Adj_Yellow_Wait', color_tolerance=30)
        if found_yellow:
            return True
        search_attempts += 1
